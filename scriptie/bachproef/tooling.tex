...
\section{Flutter}
De Flutter command-line tool is hoe ontwikkelaars (en IDE’s aangestuurd door ontwikkelaars) omgaan met Flutter. Voor het kunnen werken met Flutter moet dus eerst de Flutter SDK geïnstalleerd worden. Deze SDK bevat onder andere een aantal commando’s die kunnen uitgevoerd worden in de terminal. Flutter commando’s worden voorafgegaan door het flutter keyword. Zo weet het besturingssysteem dat een commando wordt gebruikt van de flutter tool. De dart-tool is een command line interface voor de Dart SDK. De tool is beschikbaar, ongeacht hoe de Dart SDK gedownload werd (expliciete downloadt of alleen de Flutter SDK downloadt). Indien het mogelijk is om de flutter tool te gebruiken is dit het betere alternatief.
...
\section{Conclusie}
Zowel in Flutter als Android kan het meeste vanop de command line gebeuren. Echter omgekeerd is dat niet het geval. Aangezien Flutter geen toegewijde IDE heeft moeten sommige commando's uitgevoerd worden in de terminal. Terwijl Android via de IDE de meeste commando's kan uitvoeren. De Flutter IDE commando's zijn eerder beperkt.